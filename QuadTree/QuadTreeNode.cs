...
namespace Blueberry
{
    ...
    public class QuadTreeNode<T> where T : IQuadTreeItem
    {
        ...
        public List<T> Query(Rectangle queryArea)
        {
            ...
            if (this.Area.IntersectsWith(queryArea))
            {
                ...
                {
					if (queryArea.IntersectsWith(item.Collider.Bounds))
                        results.Add(item);
                }
                if (partitioned)
                {
        ...
        public List<T> Query(Point queryPoint)
        {
            ...
            if (Area.Contains(queryPoint))
            {
                ...
                {
                    if (item.Collider.Contains(queryPoint))
                        results.Add(item);
                }
                if (partitioned)
                {
        ...
        public void Query(ref Rectangle queryArea, List<T> results)
        {
            ...
            if (this.Area.IntersectsWith(queryArea))
            {
                foreach (T item in this.NodeContent)
                {
					if (item.Collider.Collides(queryArea))
                        results.Add(item);
                }
                if (partitioned)
                {
        ...
        public QuadTreeNode<T> FindItemNode(T item)
        {
            ...
            else if (partitioned) // иначе проверяем подузлы
            {
                ...
                if (leftTopNode.Area.Contains(item.Collider))
                {
                    n = leftTopNode.FindItemNode(item);
                }
                if (n == null && leftBottomNode.Area.Contains(item.Collider))
                {
                    n = leftBottomNode.FindItemNode(item);
                }
                if (n == null && rightBottomNode.Area.Contains(item.Collider))
                {
                    n = rightBottomNode.FindItemNode(item);
                }
                if (n == null && rightTopNode.Area.Contains(item.Collider))
                {
        ...
        protected bool TryInsertInChild(T item)
        {
            ...
            if (partitioned)
            {
                ...
                else if (rightTopNode.Area.Contains(item.Collider))
                    rightTopNode.Insert(item);
                else if (leftBottomNode.Area.Contains(item.Collider))
                    leftBottomNode.Insert(item);
                else if (rightBottomNode.Area.Contains(item.Collider))
        ...
        protected bool TryPushItemUp(int i)
        {
            ...
            {
                if (node.Area.Contains(NodeContent[i].Collider))
                {
        ...
        public bool RemoveItem(T item)
        {
            ...
            else if (Area.Contains(item.Collider))
            {
                        ...
                        if (!Area.Contains(i.Collider))
                        {
                            ...
                            if (!TryPushItemUp(index))
                            {
                                ...
                                tree.Resize(new Rectangle(
									Math.Min(tree.Area.Left, i.Collider.Bounds.Left),
									Math.Min(tree.Area.Top, i.Collider.Bounds.Top),
									Math.Max(tree.Area.Right, i.Collider.Bounds.Right) - Math.Min(tree.Area.Left, i.Collider.Bounds.Left),
									Math.Max(tree.Area.Bottom, i.Collider.Bounds.Bottom) - Math.Min(tree.Area.Top, i.Collider.Bounds.Top)));
                            }
                        }
                    }
                }
    ...
    {
		IQuadTreeCollider Collider { get; }
