...
namespace Blueberry
{
    ...
    public class QuadTree<T> where T : IQuadTreeItem
    {
        ...
        public void Insert(T item)
        {
            ...
            if (!rootNode.Area.Contains(item.Collider))
            {
                // изменяем размер дерева
                Resize(new Rectangle(
					Math.Min(rootNode.Area.Left, item.Collider.Bounds.Left),
					Math.Min(rootNode.Area.Top, item.Collider.Bounds.Top),
					Math.Max(rootNode.Area.Right, item.Collider.Bounds.Right) - Math.Min(rootNode.Area.Left, item.Collider.Bounds.Left),
					Math.Max(rootNode.Area.Bottom, item.Collider.Bounds.Bottom) - Math.Min(rootNode.Area.Top, item.Collider.Bounds.Top)));
            }
