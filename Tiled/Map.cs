...
namespace Blueberry.Tiled {
    ...
    public class Map {
        ...
        internal static Texture _whiteTexture = null;
        ...
        /// <summary>
        /// Ordered collection of image layers; first is the bottom layer
        /// </summary>
        public ImageLayerList ImageLayers;
        /// <summary>
        ...
        public void Draw(SpriteBatch spriteBatch, ref Rectangle region, Boolean drawHiddenLayers) {
            ...
            if (this.Orientation == MapOrientation.Isometric || this.Orientation == MapOrientation.Staggered) {
                tyMax = tyMax * 2 + 1;
                txMax = txMax * 2 + 1;
            }
        ...
        private void DrawLayer(SpriteBatch spriteBatch, Int32 layerID, ref Rectangle region, Int32 txMin, Int32 txMax, Int32 tyMin, Int32 tyMax, Single layerDepth) {
            for (int y = tyMin - 1; y <= tyMax; y++) {
                for (int x = txMin - 1; x <= txMax; x++) {
                    if (x >= 0 && x < this.TileLayers[layerID].Tiles.Length && y >= 0 && y < this.TileLayers[layerID].Tiles[x].Length && this.TileLayers[layerID].Tiles[x][y] != null) {
        ...
        public void DrawTileObject(SpriteBatch spriteBatch, Int32 objectLayerID, Int32 objectID, Rectangle region, Single layerDepth, Color color) {
            this.DrawTileObject(spriteBatch, this.ObjectLayers[objectLayerID].MapObjects[objectID], ref region, layerDepth, ref color);
        }
        ...
        public void DrawMapObject(SpriteBatch spriteBatch, Int32 objectLayerID, Int32 objectID, ref Rectangle region, Single layerDepth) {
            ...
            this.DrawMapObject(spriteBatch, this.ObjectLayers[objectLayerID].MapObjects[objectID], ref region, layerDepth, color);
        }
        ...
        /// <summary>
        /// Draws all images on the given image layer
        /// </summary>
        /// <param name="spriteBatch">XNA SpriteBatch instance; SpriteBatch.Begin() must be called before using this method</param>
        /// <param name="imageLayerID">Index of the layer to draw in the Map.ImageLayers collection</param>
        /// <param name="region">Region of the map in pixels currently visible</param>
        /// <param name="layerDepth">LayerDepth value to pass to SpriteBatch</param>
        public void DrawImageLayer(SpriteBatch spriteBatch, Int32 imageLayerID, Rectangle region, Single layerDepth) {
            DrawImageLayer(spriteBatch, imageLayerID, ref region, layerDepth);
        }
        ...
        /// <param name="region">Region of the map in pixels currently visible</param>
        /// <param name="layerDepth">LayerDepth value to pass to SpriteBatch</param>
        public void DrawImageLayer(SpriteBatch spriteBatch, Int32 imageLayerID, ref Rectangle region, Single layerDepth) {
            if (Map._whiteTexture == null) {
                throw new Exception("Map.InitObjectDrawing must be called before Map is loaded to enable object rendering");
            }

            for (int i = 0; i < this.ImageLayers[imageLayerID].MapImages.Length; i++) {
                Rectangle bounds = new Rectangle(0, 0, this.ImageLayers[imageLayerID].MapImages[i].ImageWidth, this.ImageLayers[imageLayerID].MapImages[i].ImageHeight);
                if (region.Contains(bounds) || region.Intersects(bounds)) {
                    DrawMapImage(spriteBatch, imageLayerID, i, ref bounds, ref region, layerDepth);
                }
            }
        }
