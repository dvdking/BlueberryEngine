...
namespace Blueberry.Tiled {
    ...
    public sealed class MapReader : ContentTypeReader<Map> {
        ...
        protected override Map Read(ContentReader input, Map existingInstance) {
            ...
            m.Orientation = input.ReadEnum<MapOrientation>();
            ...
            Int32 ilayers = input.ReadInt32();
            m.ImageLayers = new ImageLayerList();
            for (int i = 0; i < ilayers; i++) {

                m.ImageLayers.Add(new ImageLayer());
                m.ImageLayers[i].Name = input.ReadString();
                m.ImageLayers[i].Opacity = input.ReadSingle();
                m.ImageLayers[i].OpacityColor = Color.White;
                m.ImageLayers[i].OpacityColor.A = input.ReadByte();
                m.ImageLayers[i].Visible = input.ReadBoolean();
                ...
                if (input.ReadBoolean())
                {
                    ...
                    c.B = input.ReadByte();
                    c.A = input.ReadByte();
                    m.ImageLayers[i].Color = c;
                }
                else
                    m.ImageLayers[i].Color = null;

                m.ImageLayers[i].MapImages = new MapImage[input.ReadInt32()];
                for (int mi = 0; mi < m.ImageLayers[i].MapImages.Length; mi++) {

                    m.ImageLayers[i].MapImages[mi] = new MapImage();
                    m.ImageLayers[i].MapImages[mi].ImageFileName = input.ReadString();
                    m.ImageLayers[i].MapImages[mi].ImageHeight = input.ReadInt32();
                    m.ImageLayers[i].MapImages[mi].ImageWidth = input.ReadInt32();
                    m.ImageLayers[i].MapImages[mi].Name = input.ReadString();

                    if (input.ReadBoolean()) {
                        Color c = Color.White;
                        c.A = input.ReadByte();
                        m.ImageLayers[i].MapImages[mi].ImageTransparentColor = c;
                    }

                    props = input.ReadInt32();
                    m.ImageLayers[i].MapImages[mi].Properties = new Dictionary<String, Property>();
                    for (int p = 0; p < props; p++)
                    {
                        m.ImageLayers[i].MapImages[mi].Properties.Add(input.ReadString(), Property.Create(input.ReadString()));
                    }
                }

                props = input.ReadInt32();
                m.ImageLayers[i].Properties = new Dictionary<String, Property>(props);
                for (int p = 0; p < props; p++)
                {
                    m.ImageLayers[i].Properties.Add(input.ReadString(), Property.Create(input.ReadString()));
                }
            }
            ...
            for (int li = 0; li < m.LayerOrder.Length; li++) {
                ...
                m.LayerOrder[li].LayerType = input.ReadEnum<LayerType>();
            }
            ...
            if (m.LoadTextures) {
                ...
                for (int i = 0; i < m.ImageLayers.Count; i++) {
                    for (int j = 0; j < m.ImageLayers[i].MapImages.Length; j++) {
                        m.ImageLayers[i].MapImages[j].Texture = input.ContentManager.Load<Texture2D>(String.Format("{0}/i{1:00}/{2:00}", input.AssetName, i, j));
                    }
                }
            }
    ...
    internal static class ContentReaderExtensions {
        public static TEnum ReadEnum<TEnum>(this ContentReader @this) where TEnum : struct, IComparable {
            ...
            return (TEnum)Enum.ToObject(type, @this.ReadInt32());
        }

        public static object ReadEnum(this ContentReader @this, Type enumType) {
            if (!IsEnum(enumType))
                throw new InvalidOperationException();

            return Enum.ToObject(enumType, @this.ReadInt32());
        }
